...
/*   Updated: 2024/10/30 14:03:23 by vzuccare         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_newmap_player(t_game *game, t_map *new_map, t_map *map)
{
	...
	if (!new_map->player->dir)
		exit_close_msg(game->fd, ERR_MALLOC, game, NULL);
	new_map->player->pos->x = -2;
	new_map->player->pos->y = -2;
...
t_map	*fill_with_space(t_map *map, t_game *game)
{
	...
	{
		new_map->map[y] = malloc(sizeof(char) * ((ssize_t)map->map_x + 2));
		if (!new_map->map[y])
		{
			while (--y >= 0)
				free(new_map->map[y]);
